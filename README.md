...
呼び出し側はすべて可変タグ `@v1` を参照する。タグの付け替えは
[`.github/workflows/move-v1.yml`](./.github/workflows/move-v1.yml) が
main への push のたびに自動でやるので、マージすればそのまま全リポジトリに届く。
...
呼び出し側が `id-token: write` を渡す先を可変タグで参照しているので、本来はコミット
SHA で固定するところ。今どちらのリポジトリも書き込めるのはオーナーだけで、タグを
書き換えられる相手は呼び出し側のワークフローファイル自体も書き換えられるため、
SHA 固定で防げる相手がいない。両方のリポジトリの書き込み権限者が一致しなくなったら
SHA 固定に切り替える。
